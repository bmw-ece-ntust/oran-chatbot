...
def update_vectorstore(folder, config_name, status=None):
    ...
    if not os.path.exists(os.path.join(folder, "vectorstore_nv")):
        print("Vector DB not found. Please create a vector DB.")
        return 1
    ...
    chunk_to_remove="© 2024 by the O-RAN ALLIANCE e.V. Your use is subject to the copyright statement on the cover page of this specification"
    for each in raw_documents:
        each.page_content = each.page_content.replace(chunk_to_remove, "")
